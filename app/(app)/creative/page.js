      ...
      return (
        <div className="rounded-xl p-4 mb-3" style={{background:"#0d1120",border:"1px solid #1e2a48"}}>
          ...
          <div className="rounded-lg p-4" style={{background:"#111827",border:"1px solid #1e2a48"}}>
            ...
            <div className="text-sm text-text-primary leading-relaxed">{ad.descriptions?.slice(0,2).join(' ') || desc}</div>
          </div>
        </div>
      )
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-text-primary tracking-tight">Ad creative & tracking</h1>
          <p className="text-sm text-text-secondary mt-0.5">Agent 4 — AI ad copy, creative briefs and full tracking setup guide</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 mb-6 border-b border-surface-border">
        {[['ads','Ad creative'],['tracking','Tracking setup']].map(([tab,label]) => (
          <button key={tab} className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors flex items-center gap-1.5 ${activeTab===tab ? 'border-brand-gold text-brand-gold' : 'border-transparent text-text-muted hover:text-text-secondary'}`} onClick={() => setActiveTab(tab)}>
      ...
      {activeTab === 'ads' && (
        <>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div><label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">Ad type</label>
              <select className="select" value={adType} onChange={e => setAdType(e.target.value)}>{AD_TYPES.map(t => <option key={t}>{t}</option>)}</select>
            </div>
            <div><label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">Objective</label>
            ...
            <div><label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">Product / service</label>
              <input className="input" placeholder="What are you advertising?" value={product} onChange={e => setProduct(e.target.value)}/>
            </div>
            <div><label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">Key USPs</label>
              <input className="input" placeholder="e.g. Free delivery, 24/7 support" value={usp} onChange={e => setUsp(e.target.value)}/>
            </div>
            <div><label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">CTA</label>
            ...
            <div><label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">Tone</label>
            ...
            <>
              <div className="flex gap-1 mb-4 border-b border-surface-border">
                {[['copy','Ad copy (3 variants)'],['mockup','Creative mockups']].map(([tab,label]) => (
                  <button key={tab} className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeAdTab===tab ? 'border-brand-gold text-brand-gold' : 'border-transparent text-text-muted hover:text-text-secondary'}`} onClick={() => setActiveAdTab(tab)}>{label}</button>
                ))}
              </div>
              ...
              {activeAdTab === 'copy' && (
                <div className="space-y-4">
                  ...
                  {(ads.ads||[]).map((ad, i) => (
                    <div key={i} className="card p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div><span className="text-sm font-bold text-text-primary">{ad.variant}</span><span className="ml-2 text-xs text-text-secondary">— {ad.angle}</span></div>
                      ...
                      <div className="space-y-3">
                        {ad.headlines?.length > 0 && (
                          <div><p className="text-[10px] font-medium text-text-secondary uppercase tracking-wider mb-1.5">Headlines</p>
                            {ad.headlines.map((h,j) => <div key={j} className="flex items-center justify-between rounded px-3 py-2 mb-1" style={{background:"#111827",border:"1px solid #1e2a48"}}><span className="text-sm text-text-primary">{h}</span><div className="flex items-center gap-2 shrink-0 ml-2"><span className={`text-[10px] ${h.length>30?'text-status-red':'text-text-secondary'}`}>{h.length}/30</span><button onClick={()=>copy(h,`h-${i}-${j}`)} className="text-text-muted hover:text-text-primary">{copied===`h-${i}-${j}`?<Check size={11}/>:<Copy size={11}/>}</button></div></div>)}
                          </div>
                        )}
                        {ad.descriptions?.length > 0 && (
                          <div><p className="text-[10px] font-medium text-text-secondary uppercase tracking-wider mb-1.5">Descriptions</p>
                            {ad.descriptions.map((d,j) => <div key={j} className="flex items-center justify-between rounded px-3 py-2 mb-1" style={{background:"#111827",border:"1px solid #1e2a48"}}><span className="text-sm text-text-primary">{d}</span><div className="flex items-center gap-2 shrink-0 ml-2"><span className={`text-[10px] ${d.length>90?'text-status-red':'text-text-secondary'}`}>{d.length}/90</span><button onClick={()=>copy(d,`d-${i}-${j}`)} className="text-text-muted hover:text-text-primary">{copied===`d-${i}-${j}`?<Check size={11}/>:<Copy size={11}/>}</button></div></div>)}
                          </div>
                        )}
                  ...
                  {ads.creative_notes && <div className="card p-4" style={{background:"#111827"}}><p className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-2">Notes for design team</p><p className="text-sm text-text-primary">{ads.creative_notes}</p></div>}
                </div>
              )}
              ...
              {activeAdTab === 'mockup' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-sm text-text-secondary">Visual previews of each ad variant. Use as reference when designing in Canva.</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {(ads.ads||[]).map((ad, i) => <div key={i}><p className="text-xs font-semibold text-text-secondary mb-2">{ad.variant} — {ad.angle}</p><AdMockup ad={ad} adType={adType}/></div>)}
                  </div>
                  <div className="mt-4 p-4 rounded-lg" style={{background:"#111827",border:"1px solid #1e2a48"}}><p className="text-xs text-text-secondary"><strong style={{color:"#e8eaf6"}}>How to use:</strong> Copy the ad copy from the "Ad copy" tab → paste into your Canva design → use the creative direction as a guide for visuals.</p></div>
                </div>
              )}
            </>
          )}
        ...
        <div>
          <div className="card p-4 mb-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-text-primary mb-1">Generate tracking setup guide</p>
                <p className="text-xs text-text-secondary">Full step-by-step guide for GTM, GA4, conversion tracking and pixel setup{client ? ` for ${client.name}` : ''}.</p>
              </div>
              <div className="flex items-center gap-3">
                <div>
                  <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1">Platform focus</label>
                  <div className="flex gap-2">
                    {['google','meta','both'].map(p => <button key={p} onClick={() => setTrackingPlatform(p)} className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors capitalize ${trackingPlatform===p ? 'bg-surface-tertiary text-brand-gold border-brand-gold/40' : 'bg-surface-secondary text-text-muted border-surface-border-light'}`}>{p === 'both' ? 'Both' : p === 'google' ? 'Google' : 'Meta'}</button>)}
                  </div>
                </div>
          ...
          {!clientId && <div className="card p-8 text-center"><p className="text-sm text-text-secondary">Select a client above to generate their tracking setup guide</p></div>}
            ...
            <div className="space-y-3">
              {tracking.key_events_to_track?.length > 0 && (
                <div className="card p-4">
                  <p className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-2">Key events to track</p>
                  <div className="flex flex-wrap gap-2">{tracking.key_events_to_track.map((e,i) => <span key={i} className="text-xs bg-status-blue-bg text-status-blue px-2.5 py-1 rounded-lg font-mono">{e}</span>)}</div>
                </div>
              )}

              {tracking.gtm_tags_needed?.length > 0 && (
                <div className="card p-4">
                  <p className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-2">GTM tags needed</p>
                  <div className="flex flex-wrap gap-2">{tracking.gtm_tags_needed.map((t,i) => <span key={i} className="text-xs bg-status-amber-bg text-status-amber px-2.5 py-1 rounded-lg">{t}</span>)}</div>
                </div>
              )}
                ...
                <div key={i} className="card overflow-hidden">
                  <button className="w-full flex items-center justify-between p-4" onClick={() => setExpandedTracking(expandedTracking === i ? null : i)}>
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm font-bold text-text-primary">{platform.platform}</p>
                    </div>
                    {expandedTracking === i ? <ChevronUp size={14} className="text-text-secondary"/> : <ChevronDown size={14} className="text-text-secondary"/>}
                  </button>
                  {expandedTracking === i && (
                    <div className="border-t border-surface-border p-4 space-y-4">
                      {(platform.steps||[]).map((s,j) => (
                        <div key={j} className="flex gap-3">
                          <div className="w-6 h-6 rounded-full bg-surface-tertiary text-brand-gold text-[11px] font-medium flex items-center justify-center shrink-0 mt-0.5">{s.step}</div>
                          <div>
                            <p className="text-sm font-semibold text-text-primary mb-0.5">{s.action}</p>
              ...
              {tracking.verification_checklist?.length > 0 && (
                <div className="card p-4">
                  <p className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-3">Verification checklist</p>
                  <div className="space-y-2">
                    {tracking.verification_checklist.map((item,i) => (
                      <div key={i} className="flex items-start gap-2">
                        <div className="w-4 h-4 border border-surface-border-light rounded mt-0.5 shrink-0"></div>
