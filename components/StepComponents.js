      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step.key} className="flex items-center">
            <div
              className={`flex items-center gap-2 cursor-pointer ${locked ? 'opacity-40 cursor-not-allowed' : ''}`}
            ...
            >
              ...
              <span className={`text-xs font-semibold hidden sm:block ${
                done ? 'text-status-green' : active ? 'text-brand-gold' : 'text-text-secondary'
              }`}>
                {step.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`w-8 h-px mx-2 ${done ? 'bg-status-green/40' : 'bg-surface-border-light'}`}/>
            )}
          </div>
        )
      })}
  ...
  return (
    <div className="next-bar">
      <div>
        ...
        <p className="text-xs text-text-secondary mt-0.5">Next step: {next.label}</p>
      </div>
      <button
...
export function LockedState({ message }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <p className="text-sm font-semibold text-text-secondary mb-1">Step locked</p>
      <p className="text-xs text-text-secondary max-w-xs">{message || 'Complete the previous step first to unlock this agent.'}</p>
    </div>
  )
}
